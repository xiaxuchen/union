...
//            //根据事件更新用户上次使用时间
//            updateUseTime(wx,openId);
            //根据用户信息发送客服信息
//            sendKfMessage(wx,openId);
//            if(openId.equals("o1U3Tjj8m_Kqq9tJzT7B10Uj4NoA")) {
//                WxXmlOutMessage wxXmlOutMessage = new WxXmlOutTextMessage();
//                wxXmlOutMessage.setFromUserName(wx.getToUserName());
//                wxXmlOutMessage.setToUserName(wx.getFromUserName());
//                ((WxXmlOutTextMessage) wxXmlOutMessage).setContent(wx.getContent());
//                wxXmlOutMessage.setMsgType(WxConsts.CUSTOM_MSG_TRANSFER_CUSTOMER_SERVICE);
//                wxXmlOutMessage.setCreateTime(System.currentTimeMillis());
//                out.println(wxXmlOutMessage.toXml());
//            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
